...
sub add {
  ...
  #when '_bag' is already present in the hash, WebService::Solr will push this value on the list, what we don't want:
  #e.g. $store->bag->add({ _bag => "data", title => "test" }) will sent { _bag => ["data","data"],title => "test" }
  #SO WARNING: 'add' changes your hash!
  delete $data->{_bag};
